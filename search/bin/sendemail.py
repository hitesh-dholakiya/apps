...
def mail(email, argvals, ssContent, sessionKey):
    ...
    try:
        ...
        ctx = sslHelper.createSSLContextFromSettings(
            confJSON=ssContent.get('alertActions'),
...
def sendEmail(results, settings):
    ...
    if argvals.get('contentType'):
        # We get this prop shape when we send test emails
        ssContent['action.email.content_type'] = argvals.get('contentType')
    ...
    # let's capture the check and then revert the content type afterwards
    isInfoDeliveryAvailable = ssContent['action.email.content_type'].startswith(INFODEL_PREFIX)
    ssContent['action.email.content_type'] = ssContent['action.email.content_type'].replace(INFODEL_PREFIX, '')
    ...
    if isHtmlBasedContentType:
        buildHTMLBody(ssContent, resultsWithRenderedTime, settings, emailBody, email, jobCount, sessionKey, isInfoDeliveryAvailable)
...
def buildHTMLBody(ssContent, results, settings, emailbody, email, jobCount, sessionKey, isInfoDeliveryAvailable):
    ...
    if ssContent['type'] == 'view':
        ...
        if (not isInfoDeliveryAvailable) and normalizeBoolean(ssContent.get('action.email.inline')) and normalizeBoolean(
    ...
    if isInfoDeliveryAvailable:
        try:
            parameters = {
                'input_dashboard': ssContent.get('action.email.pdfview', ''),
                'namespace': settings['namespace'],
                'owner': 'admin',
                'format': 'html'
            }
            response, content = simpleRequest("infodelivery/render", sessionKey=sessionKey, getargs=parameters,
                                              timeout=PDFGEN_SIMPLE_REQUEST_TIMEOUT)

        except splunk.SplunkdConnectionException, e:
            ssContent['errorArray'].append(
                "Failed to fetch rendered contents (SplunkdConnectionException): %s" % str(e))

        try:
            content = json.loads(content)
            b64Imgs = content.get("base64Imgs")

            msgText = MIMEText('This is the alternative plain text message')
            emailbody.attach(msgText)

            resultsHTML = htmlWrapperTemplate().render(body=content.get("body"))

            for img in b64Imgs:
                msgImage = MIMEImage(base64.decodestring(img["data"]))
                msgImage.add_header("Content-ID", "<" + img["id"] + ".png>")
                msgImage.add_header("Content-Type", "image/png; name=" + img["id"] + ".png")
                msgImage.add_header("Content-Disposition", "inline; filename=" + img["id"] + ".png")
                email.attach(msgImage)
        except ValueError, e:
            ssContent['errorArray'].append("Failed to load and parse response from server")
        ...
        <table cellpadding="0" cellspacing="0" border="0" class="results" style="margin: 20px;">
            <tbody>
                ...
                % for result in results:
                    <tr valign="top">
                    % for col in cols:
                        <td style="text-align: left; padding: 4px 8px; margin-top: 0px; margin-bottom: 0px; border-bottom: 1px dotted #ccc;"><pre style="font-family: helvetica, arial, sans-serif; white-space: pre-wrap;">${result.get(col)|h}</pre></td>
                    % endfor
                    </tr>
                % endfor
            </tbody>
